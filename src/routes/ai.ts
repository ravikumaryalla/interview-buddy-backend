...
import { solveProblem, chatWithAI, chatWithAIStream, transcribeAudio, createRealtimeToken } from '../services/openai'
...
router.post('/solve', async (req, res) => {
  ...
  const cost = isReasoning ? CREDIT_COSTS.solve_reasoning : CREDIT_COSTS.solve_standard

  const deducted = await deductCredits(userId, cost, `Solve problem (${model})`)
  if (!deducted) {
  ...
  try {
    const { result, tokenCount, responseTimeMs } = await solveProblem({ problem, model, reasoningEffort })

    res.json(result)

    Promise.all([
      prisma.apiUsageLog.create({
        data: { userId, feature: 'SOLVE', creditsConsumed: cost, model, tokenCount, responseTimeMs },
      }),
...
router.post('/chat', async (req, res) => {
  ...
  const deducted = await deductCredits(userId, cost, `Chat (${body.data.model})`)
  if (!deducted) {
    res.status(402).json({ error: 'Insufficient credits' })
    return
  }
  ...
  try {
    ...
    Promise.all([
      prisma.apiUsageLog.create({
        data: { userId, feature: 'CHAT', creditsConsumed: cost, model: body.data.model, tokenCount, responseTimeMs },
      }),
      prisma.user.update({ where: { id: userId }, data: { lastActiveAt: new Date() } }),
...
router.post('/chat/stream', async (req, res) => {
  ...
  const deducted = await deductCredits(userId, cost, `Chat stream (${body.data.model})`)
  if (!deducted) {
    res.status(402).json({ error: 'Insufficient credits' })
    return
  }
  ...
  try {
    const { tokenCount, responseTimeMs } = await chatWithAIStream(
      ...
      () => { res.write('data: [DONE]\n\n'); res.end() }, // fires on finish_reason=stop, before usage chunk
    )
...
router.post('/transcribe', upload.single('audio'), async (req, res) => {
  ...
  try {
    const { text, responseTimeMs } = await transcribeAudio(req.file.buffer, req.file.originalname)

    res.json({ text })

    prisma.apiUsageLog.create({
      data: { userId, feature: 'TRANSCRIBE', creditsConsumed: cost, model: 'whisper-1', responseTimeMs },
